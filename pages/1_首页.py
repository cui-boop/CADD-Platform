...
st.markdown(
    """
    本网站不是只针对某一种药物或某一个固定靶点，而是一个通用型 CADD 数据分析平台。
...
st.markdown(
    """
    - **A 成员**：网站首页、项目管理、活性数据整理、数据接口。  
    - **B 成员**：QSAR 模型训练、分子描述符计算、活性预测。  
    - **C 成员**：ADMET 预测、Lipinski 规则、可解释性分析。  
    - **D 成员**：分子对接结果分析、综合评分、资源库和报告生成。  
    """
)