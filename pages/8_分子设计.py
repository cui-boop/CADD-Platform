...
with st.expander("候选分子输入格式说明", expanded=True):
    format_df = pd.DataFrame(
        [
            ...
            {"列名": "smiles", "是否必需": "必须", "含义": "小分子 SMILES，用于展示、结构绘图和规则优化"},
            {"列名": "target", "是否必需": "必须", "含义": "对接靶点，例如 EGFR"},
            {"列名": "docking_score", "是否必需": "必须", "含义": "分子对接得分，通常越低越好"},
        ]
    )
...
with left:
    ...
    st.write(f"**对接靶点：** {selected_row.get('target', '')}")
...
if st.button("生成结构优化分子", type="primary"):
    try:
        ...
        with tab_result:
            ...
            delta_cols = ["MolWt_delta", "LogP_delta", "TPSA_delta", "RotatableBonds_delta"]
            available_delta_cols = [c for c in delta_cols if c in design_df.columns]
            delta_df = design_df[
                design_df["designed_smiles"].astype(str).str.len() > 0
            ][["designed_compound_id"] + available_delta_cols].copy()

            if delta_df.empty or not available_delta_cols:
                st.info("暂无可展示的数值变化。")
            else:
                long_df = delta_df.melt(
                    id_vars="designed_compound_id",
                    value_vars=available_delta_cols,
                    var_name="性质指标",
                    value_name="相对母体变化值",
                ).dropna()

                long_df["性质指标"] = long_df["性质指标"].replace(
                    {
                        "MolWt_delta": "MolWt",
                        "LogP_delta": "LogP",
                        "TPSA_delta": "TPSA",
                        "RotatableBonds_delta": "RotatableBonds",
                    }
                )
                ...
                if px is not None:
                    fig = px.bar(
                        long_df,
                        x="designed_compound_id",
                        y="相对母体变化值",
                        color="性质指标",
                        barmode="group",
                        title="设计分子相对于母体分子的性质变化",
                        labels={"designed_compound_id": "设计分子", "相对母体变化值": "设计分子 - 母体分子"},
                    )
                    fig.add_hline(y=0, line_dash="dash")
                    fig.update_layout(height=430, xaxis_tickangle=-30)
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    chart_df = delta_df.set_index("designed_compound_id")
                    st.bar_chart(chart_df)

                st.caption("正值表示设计分子该性质高于母体分子，负值表示低于母体分子。")
