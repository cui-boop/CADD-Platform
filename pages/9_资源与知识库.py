...
st.markdown("""
本页面用于整理计算机辅助药物设计过程中常用的数据库、软件工具和理论知识。
...
st.subheader("5. 可写入报告的说明文字")

st.markdown("""
本项目构建了一个面向药物发现早期阶段的计算机辅助药物设计综合平台。
平台以用户上传的分子活性数据为起点，经过活性数据整理、QSAR 模型训练、ADMET 评价、
分子对接结果分析和综合评分，最终输出候选分子的推荐等级和分析报告。

资源与知识库模块用于整理本平台涉及的主要数据库、算法和工具来源，
包括 PubChem、ChEMBL、PDB、UniProt、RDKit、AutoDock Vina 和 PyMOL 等。
该模块既可以帮助用户理解平台各功能的理论基础，也可以为后续真实药物设计任务提供数据来源和工具参考。
""")
