...
def home_page():
    ...
    page_files = load_pages()

    if not page_files:
        st.warning("未检测到 pages 文件夹下的功能页面。请确认项目目录中存在 pages 文件夹，并且其中包含 .py 页面文件。")
    else:
        for row_start in range(0, len(page_files), 3):
            row_pages = page_files[row_start: row_start + 3]
...
for page_file in page_files:
    page_title = clean_page_title(page_file)
    page_icon = get_page_icon(page_title)
...
current_page = st.navigation(nav_pages)
current_page.run()